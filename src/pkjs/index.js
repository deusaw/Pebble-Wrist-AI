...
var TTS_SEND_DELAY_MS = 35;      // 350B/35ms≈10KB/s，略高于 8KB/s 播放，避免突发灌满再 PAUSE
...
function ttsSendNext(sessionId) {
  ...
  if (ttsCurrentSentence && ttsCurrentOffset < ttsCurrentSentence.length) {
    ...
    // 稳定节奏发送，略高于播放消费速度，减少 PAUSE/RESUME 循环和环形缓冲大幅震荡。
    setTimeout(function() { ttsSendNext(sessionId); }, TTS_SEND_DELAY_MS);
    return;
  }
